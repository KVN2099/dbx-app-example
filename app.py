...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    html.Div(config.sidebar_header_text, className="sidebar-header-text"),
    ...
    html.Div([
        html.Div([
            ...
            html.Div([
                ...
                className="genie-logo-container-header"),
                    ...
                    html.Div(id="welcome-description", 
                            ...
                            children=config.welcome_description),
                    ...
                    dbc.Modal([
                        ...
                        dbc.ModalBody([
                            html.Div([
                                ...
                                html.Small(
                                    "Este título aparece en la parte superior de tu pantalla de bienvenida",
            ...
            html.Div([
                ...
                html.Div(config.disclaimer_text, className="disclaimer-fixed")
...
def format_sql_query(sql_query):
    """Da formato a la consulta SQL usando la librería sqlparse"""
    formatted_sql = sqlparse.format(
        sql_query,
        keyword_case='upper',  # Convierte las palabras clave a mayúsculas
        identifier_case=None,  # Preserva el caso de los identificadores
        reindent=True,         # Añade una indentación adecuada
        indent_width=2,        # Ancho de indentación
        strip_comments=False,  # Preserva los comentarios
        comma_first=False      # Comas al final de la línea, no al principio
    )
    ...
    try:
        ...
        if isinstance(response, str):
            ...
            if query_text is not None:
                ...
                #     html.Div([
                #         html.Button([
                #             html.Span("Mostrar código", id={"type": "toggle-text", "index": query_index})
                #         ], 
        ...
        error_msg = f"Lo siento, se ha producido un error: {str(e)}. Inténtalo de nuevo más tarde."
...
def handle_modal_actions(save_clicks, close_clicks,
    ...
    if trigger_id == "close-modal":
        ...
        # Save the changes
        title = new_title if new_title else config.welcome_title
        description = new_description if new_description else config.welcome_description
        suggestions = [
            s1 if s1 else (config.suggestions[0] if len(config.suggestions) > 0 else ""),
            s2 if s2 else (config.suggestions[1] if len(config.suggestions) > 1 else ""),
            s3 if s3 else (config.suggestions[2] if len(config.suggestions) > 2 else ""),
            s4 if s4 else (config.suggestions[3] if len(config.suggestions) > 3 else "")
        ]
